...
#     Collects up to 3 'bad' frames for annotation (e.g., slouched or tilted).
#     """
...
#     slouch_flags = 0
#     valid = 0
#     bad_frames = []  # Collect (frame_idx, issue) for up to 3 bad examples
...
#         if angle_deg < -180: angle_deg += 360  # Normalize to -180 to 180
...
#     score -= (slouch_pct * 50.0)  # Heavier for formal confidence (up from 40)
#     score -= (tilt_pct * 20.0)  # Lighter, as minor tilt is common
#     score = max(0.0, round(score))  # 0-100 range for granularity
...
#     feedbacks.append("You’re doing well—keep building on this strong foundation!")
...
#         "feedback": feedbacks,
#         "bad_frames": bad_frames  # For annotation in pipeline (e.g., draw green ideal lines)
#     }
...
#     fb.append("You’re doing great—keep refining those movements!")
...
#     Explicitly detects anger (tight mouth, down-turned corners) and nervousness (rapid eye shifts, small mouth).
#     """
#     frames = processed.get("frames", [])
#     meta = processed.get("meta", {})
#     validation = meta.get("validation", {})
...
#     smile_flags = []
#     eye_contact_flags = []
#     expr_states = []  # 0=neutral, 1=smile, 2=anger, 3=nervous
#     anger_flags = []
#     nervous_flags = []
...
#         # Anger detection: tight mouth or down-turned corners
#         center_y = (mt_px[1] + mb_px[1]) / 2
#         is_anger = (mouth_h < 0.05 * height) or (ml_px[1] > center_y + 0.01 * height and mr_px[1] > center_y + 0.01 * height)
#         anger_flags.append(is_anger)
...
#     anger_pct = sum(anger_flags) / max(1, len(anger_flags))
...
#     if anger_pct > 0.2:
#         score -= (anger_pct - 0.2) * 37.5  # Strong penalty for anger
...
#     if anger_pct > 0.2:
#         fb.append("Some expressions seem tense or angry. Aim for a calm, neutral tone.")
#     if nervous_pct > 0.2 or is_nervous_eye:
...
#     fb.append("You’re doing great—keep refining your presence!")

#     logger.debug(f"Expressions: anger_pct={anger_pct}, nervous_pct={nervous_pct}, eye_offset_std={eye_offset_std}")
...
#         "anger_percent": round(anger_pct * 100, 1),
...
#     """
#     Top-level API: Honors analysis_mode from validation, returns posture, gestures, expressions, and overall average.
#     """
...
#     overall = int(round(np.mean(scores))) if scores else None

#     overall_feedback = [part.get("feedback") or part.get("message", []) 
#                         for part in (posture, gestures, expressions) 
#                         if part.get("feedback") or part.get("message")]
#     overall_feedback = [item for sublist in overall_feedback for item in sublist]  # Flatten list
#     if len(scores) < 3:
#         overall_feedback.insert(0, "Some parts couldn’t be analyzed due to limited visibility. Try keeping your body and face in the frame!")
...
#             "feedback": overall_feedback if overall_feedback else ["Analysis completed—nice effort!"]
#         }
#     }
